...
export default function Questions({
  ...
  const [, setHistory] = useState<Question[]>([]);
  ...
  const handleNext = useCallback(() => {
    setHistory((prevHistory) => {
      const updatedHistory = [...prevHistory, current];
      ...
      const next = (() => {
        if (isCorrect) {
          if (currentDiff === "easy")
            return dummyQuestions.find(
              (q) => q.difficulty === "medium" && !updatedHistory.includes(q)
            );
          if (currentDiff === "medium")
            return dummyQuestions.find(
              (q) => q.difficulty === "hard" && !updatedHistory.includes(q)
            );
        } else {
          if (currentDiff === "hard")
            return dummyQuestions.find(
          ...
          if (currentDiff === "medium")
            return dummyQuestions.find(
              (q) => q.difficulty === "easy" && !updatedHistory.includes(q)
            );
        }
        return null;
      })();
  ...
  }, [selected, current, setQuestionIndex]);
